...
def on_click(event):
    ...
    axGraph.cla()
    draw_the_map()
    axMap.set_title(f"C: ({x:.1f},{y:.1f}) - Nedbørsmengde ut i fra gitt koordinat")
    ...
    gjennomsnitt_nedbor = np.mean(y_pred)
    axGraph.axhline(y=gjennomsnitt_nedbor, color='red', linestyle='--', label='Gjennomsnitt nedbør for punktet')
    axGraph.legend()
